...
function StegRad({ steg, forst }: { steg: Step; forst: boolean }) {
  ...
  return (
    <li className="border-b border-linesoft last:border-b-0">
      ...
      >
        <span
          className={`w-8 shrink-0 text-[17px] font-bold tabular-nums ${
        ...
        <span className="min-w-0 flex-1">
          <span
            className={`block text-[13.5px] label-caps tracking-[0.09em] ${
              last ? "text-dim" : "text-bone"
            }`}
          >
            {steg.namn}
          </span>
          <span className="mt-1 block truncate text-[11.5px] text-dim">{steg.kort}</span>
        </span>
        <span className="hidden shrink-0 text-right sm:block">
          <span className="block text-[11px] tabular-nums text-body">
            {steg.maxPoang === 0
              ? "inga poäng"
          ...
          {steg.datum ? (
            <span className="mt-0.5 block text-[10px] text-dim">{steg.datum}</span>
          ) : null}
        </span>
        ...
        <div className="expand px-5 pb-5 sm:pl-[68px]">
          {last ? (
            <div className="border border-dashed border-line bg-page/35 px-4 py-3.5">
              ...
              <p className="mt-2 max-w-[74ch] text-[12.5px] leading-[1.65] text-body">
                {steg.kravs}
              </p>
              ...
              <div>
                ...
                <ul className="mt-2.5 space-y-2">
                  {steg.gjordes?.map((g, i) => (
                    <li key={i} className="flex gap-2.5 text-[12px] leading-[1.6] text-body">
                ...
                <dl className="mt-2.5 space-y-2.5">
                  {steg.resultat?.map((r) => (
                    <div key={r.etikett} className="border-l border-line pl-3">
                      <dt className="text-[10px] label-caps tracking-[0.11em] text-dim">
                        {r.etikett}
                      </dt>
                      <dd className="mt-1 text-[12.5px] leading-[1.55] text-bone">
                        {r.varde}
                      </dd>
                    </div>
                  ))}
            ...
            <Panel key={fas.namn} className="rise">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-5 py-3">
                <div className="flex items-baseline gap-3">
                  <h2 className="text-[14px] label-caps tracking-[0.15em] text-bone">
                    {fas.namn}
                  </h2>
                  <span className="text-[10.5px] text-dim">Steg {fas.steg}</span>
                </div>
                <div className="flex items-center gap-3 text-[10px] label-caps tracking-[0.11em] text-dim">
                  <span>
